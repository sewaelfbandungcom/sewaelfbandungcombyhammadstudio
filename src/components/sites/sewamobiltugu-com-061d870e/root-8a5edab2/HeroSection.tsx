      ...
      <div className="relative min-h-[790px] overflow-hidden rounded-b-[52px] bg-[#101B38] sm:rounded-b-[68px] lg:min-h-[800px] lg:rounded-b-[92px]">
        ...
        <div className="site-container relative z-10 flex min-h-[790px] items-start pb-[205px] pt-[152px] sm:pt-[166px] lg:min-h-[800px] lg:items-center lg:pb-[175px] lg:pt-[120px]">
          ...
          <div className="hero-reveal hero-delay-4 mt-8 flex flex-wrap gap-3">
            <a href={bookingUrl()} target="_blank" rel="noreferrer" className="inline-flex h-12 items-center gap-2 rounded-xl bg-[#50C710] hover:bg-[#43aa0c] px-6 text-sm font-bold text-white shadow-[0_12px_28px_rgba(80,199,16,.35)] transition hover:-translate-y-0.5">Pesan via WhatsApp <ArrowRight className="size-4" /></a>
            <a href="#armada" className="inline-flex h-12 items-center gap-2 rounded-xl border border-white/30 bg-white/10 px-6 text-sm font-semibold text-white backdrop-blur transition hover:bg-white/20">Lihat Daftar Armada <ArrowRight className="size-4" /></a>
          </div>
          <div className="hero-reveal hero-delay-5 mt-7 flex w-fit max-w-full flex-wrap items-stretch overflow-hidden rounded-2xl border border-white/15 bg-[#101B38]/80 shadow-[0_16px_42px_rgba(0,0,0,.28)] backdrop-blur-md">
            <div className="min-w-[155px] border-b border-white/10 px-4 py-3 sm:border-b-0 sm:border-r bg-[#1237B8]/30">
            ...
            {servicePromos.map(({ label, icon: Icon }) => (
              <span key={label} className="flex min-w-[112px] items-center gap-2 border-l border-white/10 px-3.5 py-3 text-[10px] font-semibold text-slate-200">
                <Icon className="size-4 text-[#2857E8]" />{label}
              </span>
            ))}
          </div>
          <div className="hero-reveal hero-delay-6 mt-5 flex flex-wrap items-center gap-x-5 gap-y-3">
            <div className="flex -space-x-2">{["AG", "EB", "ST", "ZG"].map((initials) => <span key={initials} className="grid size-8 place-items-center rounded-full border-2 border-[#101B38] bg-gradient-to-br from-[#1237B8] to-[#6920CC] text-[8px] font-bold text-white shadow-sm">{initials}</span>)}</div>
            ...
            <span className="flex items-center gap-2 text-[11px] font-medium text-slate-200"><span className="grid size-5 place-items-center rounded-full border border-[#50C710] bg-[#50C710]/20 text-[#50C710]"><Check className="size-3" /></span>Unit Euro4 Terawat</span>
          </div>
          </div>
        </div>
      </div>
